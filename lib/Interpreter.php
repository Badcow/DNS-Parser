...
class Interpreter
{
    ...
    /**
     * Return an associative array of the data [0]
    ...
    public static function getResourceNameFromLine($line)
    {
        ...
        if (false === $result) {
            throw new ParseException(sprintf('Could not get resource name from data "%s".', $line));
        }
    ...
    /**
     * Get the class from a data (if there is one)
     *
     * @param string $line
     * @return string|null
     * @throws ParseException
    ...
    public static function getClassFromLine($line)
    {
        ...
        if ($result === false) {
            throw new ParseException(sprintf('Could not get class from data "%s".', $line));
        }
        ...
        if (!array_key_exists($class, Classes::$classes)) {
            throw new ParseException(sprintf('Class "%s" in data "%s" is not a valid class.', $class, $line));
        }
    ...
    public static function expand($zoneData)
    {
        ...
        while ($iterator->valid()) {
            ...
            $lines->addLine(self::parseLine($iterator));
            $iterator->next();
        }
